...
4. `generate-tools` — chains two steps into `src/tools.ts`:
   ...
   - `scripts/enrich-tool-metadata.mjs` then adds two more disambiguation notes to descriptions on top of that: a "Scope: ..." sentence + named alternative for tool pairs that share a resource at two *scopes* (e.g. `getAllWorkItems` vs. `getWorkItems`, `getGlobalEnumeration` vs. `getProjectEnumeration`), and a "Cardinality: ..." sentence + named alternative for tool pairs that share a resource path and HTTP method but target a single item by ID vs. the full collection/a batch (e.g. `deleteCollection` vs. `deleteCollections`) — a different axis from scope, so a tool can carry both notes.
   ...
   - All three enrichment passes are idempotent — safe to rerun directly against `src/tools.ts` (see `scripts/lib/entry-utils.mjs` for the shared text-editing helpers).
